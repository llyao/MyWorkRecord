...
namespace Quick.Code
{
    public class GenerateCodeWindow : EditorWindow
    {
        ...
        private string codeTaskTextCS= "using UnityEngine;\nusing GameILruntime.Core;\n\npublic class {0} : UITask\n{{\n\tpublic {0}()\n\t{{\n\t\tprefabName = \"{1}\";\n\t}}\n\tpublic override void OnLoadedCompleted(GameObject go)\n\t{{\n\t\tview = go.AddComponent<{2}>();\n\t\t// 加载完成，逻辑代码写在下面\n\t}}\n\n}}";
        ...
        //需要生成字段的控件,可通过toggle选择
        private Dictionary<string, bool> genteredEventWidgets = new Dictionary<string, bool>();
        //UI 类名
        ...
        private string btnCallbackActionSerilCode { get { return selectedBar == 0 ? CodeConfig.btnCallbackActionSerilCode : CodeConfig.btnCallbackSerilCodeLua; } }

        private string setLabelSerilCode { get { return selectedBar == 0 ? CodeConfig.setLabelSerilCode : CodeConfig.setLabelSerilCodeLua; } }
        ...
        private string eventCallbackActionSerilCode { get { return selectedBar == 0 ? CodeConfig.eventCallbackActionSerilCode : CodeConfig.eventCallbackSerilCodeLua; } }

        private string labelRegion { get { return selectedBar == 0 ? CodeConfig.labelRegion : CodeConfig.labelRegionLua; } }
        private string actionRegion { get { return selectedBar == 0 ? CodeConfig.actionRegion : CodeConfig.actionRegionLua; } }
        ...
        private void DrawFindWidget()
        {
            ...
            using (EditorGUILayout.HorizontalScope hScope = new EditorGUILayout.HorizontalScope())
            {
                ...
                if (GUILayout.Button("查找UI控件", GUILayout.Width(halfViewWidth / 2f)))
                {
                    ...
                    RecursiveUI(root.transform, (tran) =>
                    {
                        ...
                        for (int i = 0; i < widgets.Length; i++)
                        {
                            ...
                            if (widget != null && !uiWidgets.Contains(widget))
                            {
                                ...
                                if (!genteredEventWidgets.ContainsKey(typeName) || !genteredEventWidgets[typeName])
                                {
                    ...
                    uiWidgets.Sort((left, right) =>
                    {
                        int i = left.GetType().Name.CompareTo(right.GetType().Name);
                        if (i == 0)
                        {
                            return CompareDepth(left.transform, right.transform);
                        }
        ...
        private int CompareDepth(Transform left, Transform right)
        {
            Transform leftBefore = left;
            Transform rightBefore = right;

            Transform leftParent = left;
            Transform rightParent = right;
            while (leftParent != root.transform)
            {
                while (rightParent != root.transform)
                {
                    if (leftParent != rightParent)
                    {
                        rightBefore = rightParent;
                        rightParent = rightParent.parent;

                    }
                    else
                    {
                        if (leftBefore.parent == rightBefore)
                        {
                            return 1;
                        }
                        else if (rightBefore.parent == leftBefore)
                        {
                            return -1;
                        }
                        if (leftBefore.GetSiblingIndex() > rightBefore.GetSiblingIndex())
                        {
                            return 1;
                        }
                        else if (leftBefore.GetSiblingIndex() == rightBefore.GetSiblingIndex())
                        {
                            return 0;
                        }
                        else
        ...
        private void DrawCsPage()
        {
            ...
            if (GUILayout.Button("清除脚本", GUILayout.Width(halfViewWidth / 2f)))
            {
                CleanText();
            }
            ...
            using (EditorGUILayout.VerticalScope vScope = new EditorGUILayout.VerticalScope())
            {
                ...
                EditorGUILayout.LabelField("选择需要生成字段的控件:");
                DrawGenteredEventWidget();
            ...
            using (EditorGUILayout.HorizontalScope hScope = new EditorGUILayout.HorizontalScope())
            {
                if (GUILayout.Button("变量声明", GUILayout.Width(halfViewWidth / 2f)))
                {
                    BuildStatementCode();
                }
        ...
        {
            if (codeAssignText != null && codeAssignText.Length > 0)
            {
                codeAssignText.Remove(0, codeAssignText.Length);

            }
            if (codeStateText != null && codeStateText.Length > 0)
            {
                codeStateText.Remove(0, codeStateText.Length);

            }
            if (codeEventText != null && codeEventText.Length > 0)
            {
        ...
        private void DrawGenteredEventWidget()
        {
            using (EditorGUILayout.HorizontalScope hScope = new EditorGUILayout.HorizontalScope())
            {
                //筛选当前UI的事件控件
                foreach (var elem in Enum.GetValues(typeof(CodeConfig.EventGenteredWidgetType)))
                {
                    if (!genteredEventWidgets.ContainsKey(elem.ToString()))
                    {
                        genteredEventWidgets.Add(elem.ToString(), true);
                    }
                    ...
                    //    if (type.Name == elem.ToString() && !genteredEventWidgets.ContainsKey(type.Name))
                    //    {
                    //        genteredEventWidgets.Add(type.Name, true);
                    //    }
                    //}
                }

                //绘制toggle,注意不能遍历dic的同时赋值
                List<string> list = new List<string>(genteredEventWidgets.Keys);
                foreach (string wedagetName in list)
                {
                    genteredEventWidgets[wedagetName] = EditorGUILayout.ToggleLeft(wedagetName, genteredEventWidgets[wedagetName],
                        GUILayout.Width(halfViewWidth / 10f));
                }
            }
        }
        ...
        private string BuildEventCode()
        {
            ...
            // 生成所有Action 广播事件。
            codeEventText.Append(actionRegion);
            for (int i = 0; i < uiWidgets.Count; i++)
            {
                if (uiWidgets[i] == null) continue;
                    ...
                    if (uiWidgets[i].Equals(variableNameDic[vName]))
                    {
                        ...
                        if (!string.IsNullOrEmpty(variableName))
                        {
                            ...
                            if (uiWidgets[i] is Button)
                            {
                                ...
                                codeEventText.AppendFormat(btnCallbackActionSerilCode, methodName);
                            ...
                            else
                            {
                                ...
                                {
                                    codeEventText.AppendFormat(eventCallbackActionSerilCode, methodName, paramType);
                                }
                ...
                foreach (var vName in variableNameDic.Keys)
                {
                    if (uiWidgets[i].Equals(variableNameDic[vName]))
                    {
                        ...
                        if (!string.IsNullOrEmpty(variableName))
                        {
                            ...
                            if (uiWidgets[i] is Button)
                            {
                                string addEventStr = string.Format(onClickSerilCode, variableName, methodName);
            ...
            codeEventText.Replace("@@",";");
            codeEventText.Append(regionEnd);

            // set Label
            codeEventText.Append(labelRegion);
            for (int i = 0; i < uiWidgets.Count; i++)
            {
                if (uiWidgets[i] == null) continue;
                foreach (var vName in variableNameDic.Keys)
                {
                    if (uiWidgets[i].Equals(variableNameDic[vName]))
                    {
                        ...
                        {
                            string methodName = variableName.Substring(variableName.IndexOf('_') + 1);
                            if (uiWidgets[i] is Text)
                            {
                                codeEventText.AppendFormat(setLabelSerilCode, methodName, variableName);
        ...
        private void CreateCsUIScript()
        {
            ...
            path = EditorUtility.SaveFilePanel("Create Script", path, root.name+"UIView" + ".cs", "cs");
            if (string.IsNullOrEmpty(path)) return;

            int index = path.LastIndexOf('/');
            className = path.Substring(index + 1, path.LastIndexOf('.') - index - 1);
            taskName = className.Replace("UIView","UITask");
            string taskPath = path.Replace("UIView", "UITask");
            ...
            scriptBuilder.Append(CodeConfig.classEnd);
             ...
             File.WriteAllText(taskPath, string.Format( codeTaskTextCS,taskName,root.name,className), new UTF8Encoding(false));
             // File.WriteAllText(taskPath, "xxx", new UTF8Encoding(false));

            Debug.Log("脚本生成成功,生成路径为:" + taskPath);
