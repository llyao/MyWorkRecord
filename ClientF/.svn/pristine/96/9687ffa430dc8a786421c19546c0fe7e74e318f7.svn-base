...
namespace ClientCore.Platform
{
	public abstract class PlatformModule : Module
	{
		...
		public abstract double RealtimeSinceStartup
		{
			get;
		}
		...
		public virtual bool RemoveFolder(string folder)
		{
			...
			{
				Logger.Trace("Remove {0} failed: {1}", folder, ex);
				return false;
			}
		}
		...
		public virtual string[] GetDirectories(string path, string searchPattern = "*", SearchOption searchOption = SearchOption.TopDirectoryOnly)
		{
			...
			{
				Logger.Fatal("Directory.GetDirectories failed twice! path:{0}, searchPattern:{1}, err:{2}, inner err:{3}", path,
		...
		public virtual Stream OpenAppend(string path)
		{
			...
			{
				Logger.Error("OpenAppend failed, path {0}, e {1}", path ?? string.Empty, e);
				return null;
			}
		}
