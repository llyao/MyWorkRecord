...
class SheetInterpreter:
    ...
    def __init__(self, xls_file_path):
        ...
        print len( self._xls_file_path)
        print len(("xls\商品.xls".decode("utf-8")))
        #if cmp(self._xls_file_path.decode("utf-8"),"xls\商品.xls".decode("utf-8")):
         #   print("ok")
        #else:
           # print("fail----------")
        ...
        try :
            # pp = ""
            # for i in range(len(self._xls_file_path)):
            #     pp +=str(ord(self._xls_file_path[i]))
            self._workbook = xlrd.open_workbook(self._xls_file_path)
        except BaseException, e :
            print "open xls file(%s) failed!"%(e.message)
    ...
    def Interpreter(self) :
        ...
        for sheet in self._workbook.sheets():
            ...
            # 保存所有结构的名字
             ...
             self._output.append("package Config;\n")
    ...
    def _FieldDefine(self, repeated_num) :
        ...
        field_cs = str(self._sheet.cell_value(CLIENT_SERVER, self._col)).strip()
        if field_cs == "server":
            self._col += 1
            return
...
class DataParser:
    ...
    def Parse(self) :
        ...
        for sheet in self._workbook.sheets():
            ...
            for self._row in range(FIELD_DATA, self._row_count) :
            # 如果 id 是 空 直接跳过改行
                info_id = str(self._sheet.cell_value(self._row, id_col)).strip()
                if info_id == "" :
...
def Xls2Protobuf( path):
    ...
    if op == 0 or op == 1:
        ...
        print xls_file_path+"Interpreter Success!!! DataParser wait----------------"
    ...
    if op == 0 or op == 2:
        ...
        print xls_file_path+"Parse Success!!!"
...
if __name__ == '__main__' :
   ...
   # if len(sys.argv) > 2 :
    ...
    #xls_file_path = "xls\Test.xls"
    #proto_name = sys.argv[3]
    try:
        path=xls_file_path.decode('utf-8')
    except BaseException, e:
        print len(bytes(xls_file_path[1:]))
        path="."+bytes(xls_file_path[1:])[2:].decode('utf-8')
        print( path)
        #path=xls_file_path[1:].decode('utf-8')

    if op == 0 or op == 1:
        try :
            tool = SheetInterpreter(path)
            tool.Interpreter()
        except BaseException, e :
            print "Interpreter Failed!!!"
            print e
